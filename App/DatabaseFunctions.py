...
def getDoctorsByService(database, serviceID):
    ...
    if db is not None:
        ...
        result = cs.execute("SELECT * FROM doctors WHERE specialties LIKE '%"+serviceID+"%'")
        rows = result.fetchall()
        array = []
        if rows != None:
            for row in rows:
...
def getDoctorsByID(database, doctorID):
    ...
    if db is not None:
        cs = db.cursor()
        tableExists(cs, "doctors(name TEXT, ratings REAL, specialties TEXT, doctorID INTEGER PRIMARY KEY, picture TEXT)")
        result = cs.execute("SELECT * FROM doctors WHERE doctorID LIKE '%"+str(doctorID)+"%'")
        rows = result.fetchall()
        array = []
        if rows != None:
            for row in rows:
...
def getProfileForLogin(database, email, password):
    ...
    if db is not None:
        ...
        #db.commit()
        print("SELECT * FROM users WHERE email = "+email+" AND password = '"+password+"';")
        result = cs.execute("SELECT * FROM users WHERE email = "+str(email)+" AND password = '"+str(password)+"';")
        rows = result.fetchall()
        array = []
        if rows != None:
            for row in rows:
...
def getUserInfo(database,userID):
    ...
    if db is not None:
        ...
        #db.commit()
        result = cs.execute("SELECT * FROM users WHERE userID="+str(userID)+";")  #vulnerable to SQL injection
        rows = result.fetchall()
        array = []
        if rows != None:
            for row in rows:
                ...
                dic["email"] = row[1]
                dic["userID"] = row[3]
                dic["name"] = encryption.decryptBytes( dic["name"], cryptoPass, "cryptoKeys/keys_privK.pem")
                array.append(dic)
        cs.close()
        db.close()
        return json.dumps(array)
...
def postContact(database,contactInfo):
    ...
    if db is not None:
        ...
        command = "INSERT INTO contacts(title, text, date, name, email) VALUES('"+str(contactInfo["title"])+"','"+str(contactInfo["text"])+"','"+str(contactInfo["date"])+"','"+str(contactInfo["name"])+"', '"+str(contactInfo["email"])+"');"
        print(command)
        cs.execute(command)
        db.commit()
    ...
    #f = open(pat,"w")
    f = open("Contacts/"+str(contactInfo["title"]),"w")
...
def postBooking(database,bookingInfo):
    ...
    if db is not None:
        ...
        date = str(bookingInfo["date"])
        if date[0] != "'":
            date = "'" + date + "'"
        for b in bookings:
            if b["date"] == bookingInfo["date"]:
                return json.dumps(False)
    cs.execute("INSERT INTO bookings(date, doctorID, userID, serviceID) VALUES("+date+","+bookingInfo["doctorID"]+","+bookingInfo["userID"]+","+bookingInfo["serviceID"]+");")        
    db.commit()
...
def postProfile(database,profileInfo):
    ...
    if db is not None:
        cs = db.cursor()
        tableExists(cs, "users(name TEXT, email TEXT, password TEXT, userID INTEGER PRIMARY KEY)" )
        result = cs.execute("SELECT * FROM users WHERE email='%s'"%profileInfo["email"])
        rows = result.fetchall()
        if rows:
            return json.dumps(False)
        encryptedName = encryption.encryptString(profileInfo["name"], "cryptoKeys/keys_pubK.pem")
        #cs.execute("INSERT INTO users(name, email, password) VALUES("+profileInfo["name"]+","+profileInfo["email"]+","+encryptedPass+");")
        values_to_insert = (encryptedName, profileInfo["email"], profileInfo["password"])
        cs.execute("INSERT INTO users(name, email, password) VALUES(?, ?, ?)", values_to_insert)
        db.commit()
        profile = cs.execute("SELECT * FROM users WHERE email='%s'"%profileInfo["email"]).fetchall()[0][3]
        print(profile)
